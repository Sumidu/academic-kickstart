...
bib2acad <- function(bibfile = "",
  ...
  create_md <- function(x) {
    ...
    if (!file.exists(file.path(outfold, filename_md)) | overwrite) {
      ...
      img_prev_file <- paste0("publications/",x[["key"]], ".png")
      preprint_file <- paste0("pdf/",x[["key"]], ".pdf")
      slides_file <- paste0("slides/",x[["key"]], ".pdf")
      
      write(paste0("image_preview = \"",img_prev_file,"\""), fileConn, append = T)
      write("selected = false", fileConn, append = T)
      write("projects = []", fileConn, append = T)
      write("tags = []", fileConn, append = T)
      #links
      write("url_pdf = \"\"", fileConn, append = T)
      write(paste0("url_preprint = \"",preprint_file,"\""), fileConn, append = T)
      write("url_code = \"\"", fileConn, append = T)
      write("url_dataset = \"\"", fileConn, append = T)
      write("url_project = \"\"", fileConn, append = T)
      write(paste0("url_slides = \"",slides_file,"\""), fileConn, append = T)
      ...
      write(paste0("image_ = \"",img_prev_file,"\""), fileConn, append = T)
      write("caption = \"Result highlight\"", fileConn, append = T)
